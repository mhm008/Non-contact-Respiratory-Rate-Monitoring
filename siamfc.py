...
class TrackerSiamFC(Tracker):
    ...
    def init(self, image, box):
        ...
        # create hanning window  ——https://www.cnblogs.com/draven123/p/11392070.html
        ...
        # search scale factors 搜索比例因子
        self.scale_factors = self.cfg.scale_step ** np.linspace(
            -(self.cfg.scale_num // 2),
            self.cfg.scale_num // 2, self.cfg.scale_num)

        # exemplar and search sizes 示例和搜索大小
    ...
    def update(self, image):
        ...
        # print('instance_images.shape:', np.array(instance_images).shape)
        instance_images = np.stack(instance_images, axis=0)
        instance_images = torch.from_numpy(instance_images).to(
            self.device).permute([0, 3, 1, 2]).float()
        # print('instance_images.shape:', instance_images.shape)
        end_search =  datetime.datetime.now()
        # print('end_search - start_search =',end_search - start_search)
        # responses
        start_responses = datetime.datetime.now()
        with torch.set_grad_enabled(False):
            self.net.eval()
            start_feature = datetime.datetime.now()
            instances = self.net.feature(instance_images)
            end_feature = datetime.datetime.now()
            # print('end_feature - start_feature =', end_feature - start_feature)  #   卷积操作的时间
            responses = F.conv2d(instances, self.kernel) * 0.001
        responses = responses.squeeze(1).cpu().numpy()
        # print('responses.shape:',responses.shape)
        # print('responses', responses)

        # upsample responses and penalize scale changes 上采样响应并惩罚尺度变化
        ...
        # print('loc:',loc)
        end_responses = datetime.datetime.now()
        # print('end_responses- start_responses =', end_responses- start_responses)
    ...
    def _crop_and_resize(self, image, center, size, out_size, pad_color):
        ...
        # pad image if necessary  填充图像（如果需要）
        pads = np.concatenate((
